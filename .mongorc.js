// my personal changes, not through mongo-hacker:
DBQuery.shellBatchSize = 1
EDITOR="vim"
db = db.getSiblingDB("groceries") // connect to groceries database after startup
...
mongo_hacker_config = {
  ...
  indent:         3,                // number of spaces for indent
  sort_keys:      false,            // sort the keys in documents when displayed
  uuid_type:      'python',        // 'java', 'c#', 'python' or 'default'
  banner_message: 'Customized-Mongo-CLI ',  // banner message
  ...
  // Colors available: red, green, yellow, blue, magenta, cyan, gray, black
  colors: {
    'key':       { color: 'magenta' },
    'number':    { color: 'cyan' },
    ...
    'objectid':  { color: 'blue', underline: false, bright: false },
    'string':    { color: 'red' },
    'binData':   { color: 'yellow', bright: true },
    'function':  { color: 'magenta' },
    'date':      { color: 'blue' },
    'uuid':      { color: 'cyan' },
    'databaseNames':   { color: 'green', bright: true },
    'collectionNames': { color: 'cyan',  bright: true }
  }
}
    ...
    if (query !== "") {
        ...
        for (var command in result.commands) {
            ...
            if (regexp.test(command) || regexp.test(help)) {
                ...
                print(colorize(command, {color: 'magenta'}), Array(numSpaces).join(" "), "-", help);
            }
        }
    }
};
    ...
    try {
        ...
        if (typeof _verboseShell !== 'undefined' && _verboseShell) {
            ...
            if (time > slowms) {
                ...
                fetched += colorize(time, { color: "cyan", bright: false }) + " milliseconds";
            }
            output.push(fetched);
        }
        ...
        if ( this.hasNext() ) {
            ___it___  = this;
            output.push("More[" + colorize("true", { color: "black", bright: false }) + "]");
        }
        print(output.join(" -- "));
    }
    catch ( e ){
...
prompt = function() {
    ...
    return 'mongo> ' + db + '@' + host + '> ';
    // return colorize('mongo> ', {color: 'green'}) + db + '@' + host + '> ';
...
DB.prototype._getExtraInfo = function(action) {
    ...
    if (res) {
        ...
        if (time > slowms) {
            ...
            info += colorize(time + "ms", { color: 'blue', bright: false });
        }
        print(info);
    }
};
